      ...
      <v-row>
        ...
        <v-col cols="2">
          ...
          >
            <v-img
              ...
              width="200"
              style="just"
              :src="require('../assets/logo/nodeflix.svg')"
            ></v-img>
          </div>
        </v-col>
        <!-- TODO: TAB -->
        <v-col align-self="center" cols="7">
          ...
          >
            <v-tab
              ripple="false"
              class="custom-tab-items white-text"
              value="one"
              >Home</v-tab
            >
            <v-tab
        ...
        <v-col class="mb-4 d-flex justify-start" align-self="center" cols="1">
          ...
          >
            <template v-slot:activator="{ props }">
              <v-btn class="custom-tab-items" v-bind="props" icon>
            ...
            <v-card color="rgb(32, 32, 32, 0.8)" width="240">
              ...
              <v-list class="pt-0 mt-0" bg-color="rgb(32, 32, 32, 0.8)">
                <v-list-item>
                  ...
                  <v-btn
                    ...
                    to="/user/password"
                  >
                    <p style="margin-right: 78px" class="white-text mt-3">
                      Account
                    </p>
                  </v-btn>
                </v-list-item>
              ...
              <v-card-actions
                class="mt-0 pt-0"
                style="background-color: rgb(32, 32, 32, 0.8)"
              >
                <v-spacer></v-spacer>

                <v-btn variant="outlined" color="white" @click="menu = false">
                  Cancel
                </v-btn>
                <v-btn color="danger" variant="outlined" @click="logout">
                  Logout
                </v-btn>
              </v-card-actions>
            </v-card>
      ...
      <v-window class="pa-0" v-model="tab">
        <v-window-item value="one">
          <div class="pt-4 bg-black">
            ...
            <div class="bg-black">
              <h2
                class="d-flex justify-content-start ml-6 mb-4 font-weight-bold"
              >
                Popular
              </h2>
              <swiper
              ...
              >
                ...
                <swiper-slide
                  :key="index"
                  v-for="(item, index) in popularMovieList"
                >
                  <v-img
                    class="image-button align-items-center"
            ...
            <div class="mt-6">
              ...
              >
                <swiper-slide
                  :key="index"
                  v-for="(item, index) in topRatedMovieList"
                >
                  <v-img
                    class="image-button align-items-center"
                    :src="`https://image.tmdb.org/t/p/original${item.poster_path}`"
          ...
          <div class="pt-4 bg-black">
            <div class="bg-black">
              ...
              >
                <swiper-slide
                  :key="index"
                  v-for="(item, index) in popularTvList"
                  ><img
                    class="img-cover"
                    :src="`https://image.tmdb.org/t/p/original${item.poster_path}`"
                    alt="{{item}}"
            ...
            <div class="mt-6">
              ...
              <swiper
                ...
                class="mySwiper"
              >
                <swiper-slide :key="index" v-for="(item, index) in latestTvList"
                  ><img
                    :src="`https://image.tmdb.org/t/p/original${item.poster_path}`"
                    alt="{{item}}"
                /></swiper-slide>
            ...
            <div class="mt-6 pb-10">
              ...
              >
                <swiper-slide
                  :key="index"
                  v-for="(item, index) in topratedTvList"
                  ><img
                    :src="`https://image.tmdb.org/t/p/original${item.poster_path}`"
                    alt="{{item}}"
                /></swiper-slide>
...
export default {
  ...
  mounted() {
    this.$store.dispatch("fetchUser").catch((err) => console.log(err));

    axios
      .get(
        `https://api.themoviedb.org/3/movie/popular?api_key=${process.env.VUE_APP_API_KEY}&language=en-US&page=1`
      )
      .then((res) => {
    ...
    axios
      .get(
        `https://api.themoviedb.org/3/movie/upcoming?api_key=${process.env.VUE_APP_API_KEY}&language=en-US&page=1`
      )
      .then((res) => {
    ...
    axios
      .get(
        `https://api.themoviedb.org/3/tv/popular?api_key=${process.env.VUE_APP_API_KEY}&language=en-US&page=1`
      )
      .then((res) => {
    ...
    axios
      .get(
        `https://api.themoviedb.org/3/tv/on_the_air?api_key=${process.env.VUE_APP_API_KEY}&language=en-US`
      )
      .then((res) => {
    ...
    axios
      .get(
        `https://api.themoviedb.org/3/tv/top_rated?api_key=${process.env.VUE_APP_API_KEY}&language=en-US`
      )
      .then((res) => {
...
.swiper-slide .image-button:active {
  cursor: grabbing;
}
...
.swiper-slide .image-button .image-button-element {
  opacity: 0;
  visibility: hidden;
}

.swiper-slide:hover .image-button-element {
  opacity: 1;
  transition-delay: 700ms;
  visibility: visible;
}

.swiper-slide:hover .image-button {
  transition-delay: 300ms;
